        ...
        {presets.map(({ key, icon }) => {
          ...
          return (
            <button
              ...
              className={`
                group p-6 rounded-2xl border-2 text-left transition-all duration-300 hover:shadow-xl transform hover:scale-105 h-full min-h-[280px] flex flex-col
                ${isSelected 
              ...
              <div className="flex flex-col space-y-4 h-full">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="text-lg font-bold text-gray-900 mb-1 leading-tight">{config.name}</h4>
                  </div>
                </div>
                <p className="text-sm text-gray-700 leading-relaxed flex-shrink-0">{config.description}</p>
                  
                <div className="bg-gray-50 rounded-xl p-3 space-y-2 text-xs flex-grow">
                  {config.dimensions && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Dimensions:</span>
                      <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">{config.dimensions}</span>
                    </div>
                  )}
                  {config.max_dimension && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Max size:</span>
                      <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">{config.max_dimension}</span>
                    </div>
                  )}
                  {config.max_file_size && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Target size:</span>
                      <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">{config.max_file_size}</span>
                    </div>
                  )}
                  {config.file_size_range && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">File size:</span>
                      <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">{config.file_size_range}</span>
                    </div>
                  )}
                  {config.size_reduction && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Reduction:</span>
                      <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">{config.size_reduction}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600">Format:</span>
                    <span className="font-mono text-gray-800 bg-white px-2 py-1 rounded text-xs">
                ...
                {config.use_case && (
                  <div className="bg-gradient-to-r from-purple-100 to-teal-100 rounded-xl p-3 mt-auto">
